...
exports.categoryService = {
    ...
    getCategoryByHierarchy: async (hierarchy) => {
        try {
            const category = await Category_1.Category.findOne({ hierarchy });
            if (!category) {
                throw new ErrorResponse_1.ServiceError('Category not found by hierarchy path', 404);
            }
            return {
                success: true,
                data: category
            ...
            if (error instanceof ErrorResponse_1.ServiceError)
                throw error;
            throw new ErrorResponse_1.ServiceError(`Failed to fetch category by hierarchy: ${error.message}`, 500);
        }
    },
    createCategory: async (categoryData) => {
        try {
            ...
            if (error.code === 11000 && error.keyPattern?.hierarchy) {
                throw new ErrorResponse_1.ServiceError('A category with the same hierarchy already exists', 409);
            }
            if (error instanceof ErrorResponse_1.ServiceError)
                throw error;
    ...
    updateCategory: async (id, categoryData) => {
        try {
            ...
            if (categoryData.nom) {
                ...
                if (existingCategory) {
                    throw new ErrorResponse_1.ServiceError('Another category with this name already exists', 409);
                }
            }
            const category = await Category_1.Category.findById(id);
            if (!category) {
            ...
            if (error.code === 11000 && error.keyPattern?.hierarchy) {
                throw new ErrorResponse_1.ServiceError('Another category with the same hierarchy already exists', 409);
            }
            if (error instanceof ErrorResponse_1.ServiceError)
                throw error;
