...
exports.produitService = {
    ...
    createProduit: async (produitData) => {
        try {
            if (!produitData.nom || !produitData.prix || !produitData.category || !produitData.fournisseur) {
                throw new ErrorResponse_1.ServiceError('Name, price, category and supplier are required', 400);
            }
            ...
            const produit = new Produit_1.Produit(produitData);
            await produit.save();
            return {
                success: true,
    ...
    searchByCriteria: async (criteria = {}) => {
        try {
            ...
            const produits = await Produit_1.Produit.find({ ...query, ...category, ...fournisseur })
                .populate({
                path: 'category',
                select: 'nom'
            })
