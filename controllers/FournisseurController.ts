...
FournisseurController.post('/addresses', verifyToken, verifyFournisseurRole, async (req: Request, res: Response) => {
    try {
        const fournisseurId = req.user?.id;
        if (!fournisseurId) {
            return handleError(res, { statusCode: 401, message: 'User not authenticated' });
        }
        
        const response = await fournisseurService.addAddress(fournisseurId, req.body);
        handleResponse(res, response);
    } catch (error: any) {
        handleError(res, error);
    }
...
FournisseurController.put('/addresses/:addressId', verifyToken, verifyFournisseurRole, async (req: Request, res: Response) => {
    try {
        const fournisseurId = req.user?.id;
        ...
        const response = await fournisseurService.updateAddress(fournisseurId, addressId, req.body);
        handleResponse(res, response);
    } catch (error: any) {
        handleError(res, error);
    }
...
FournisseurController.delete('/addresses/:addressId', verifyToken, verifyFournisseurRole, async (req: Request, res: Response) => {
    try {
        const fournisseurId = req.user?.id;
        const { addressId } = req.params;
        ...
        const response = await fournisseurService.deleteAddress(fournisseurId, addressId);
        handleResponse(res, response);
    } catch (error: any) {
        handleError(res, error);
    }
...
FournisseurController.post('/onboarding/complete', verifyToken, verifyFournisseurRole, async (req: Request, res: Response) => {
    try {
        const fournisseurId = req.user?.id;
        if (!fournisseurId) {
            return handleError(res, { statusCode: 401, message: 'User not authenticated' });
        ...
        const response = await fournisseurService.completeOnboarding(fournisseurId, req.body);
        handleResponse(res, response);
    } catch (error: any) {
        handleError(res, error);
    }
...
FournisseurController.get('/onboarding/status', verifyToken, verifyFournisseurRole, async (req: Request, res: Response) => {
    try {
        const fournisseurId = req.user?.id;
        if (!fournisseurId) {
            return handleError(res, { statusCode: 401, message: 'User not authenticated' });
        ...
        const response = await fournisseurService.getOnboardingStatus(fournisseurId);
        handleResponse(res, response);
    } catch (error: any) {
        handleError(res, error);
    }
