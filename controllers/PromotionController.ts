...
PromotionController.get(
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            const promotions = await promotionService.getPromotionsByFournisseur(req.params.fournisseurId);
            handleResponse(res, promotions);
        } catch (error: any) {
            handleError(res, error);
        }
...
PromotionController.put(
    '/:id/changeStatus',verifyToken, async (req: Request & { user?: JwtPayload }, res: Response) => {
   ...
   // only verificateur can change status to REJETE or VALIDE (dans le cas ou le fournisseur a soumis la promotion et le verificateur veut la rejeter ou la valider)
    if (["REJETE","VALIDE"].includes(req.body.statut) && userRole !== 'verificateur') {
        return res.status(403).json({ message: 'Access denied. Only verificateurs can change status to REJETE or VALIDE.' });
    }
   ...
   // only fournisseur can change status to AcTIVE (dans le cas ou le verificateur a validé la promotion et le fournisseur veut l'activer apres paiement)
    if (req.body.statut === 'ACTIVE' && userRole !== 'fournisseur') {
        return res.status(403).json({ message: 'Access denied. Only fournisseur can activate promotions.' });
    }
    ...
    if (req.body.statut === 'ATT_VER' && userRole !== 'founnisseur') {
        return res.status(403).json({ message: 'Access denied. Only fournisseur can set promotions to ATT_VER.' });
    }

    try {   
        const promotion = await promotionService.changeOffreStatus(req.params.id, req.body.statut);
        handleResponse(res, promotion);
    } catch (error: any) {
        handleError(res, error);
    }
