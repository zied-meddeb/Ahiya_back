...
export const produitService = {
    ...
    updateProduit: async (id: string, produitData: Partial<IProduit>): Promise<ProduitResponse> => {
        try {
            if (!Types.ObjectId.isValid(id)) {
                throw new ServiceError('Invalid product ID format', 400);
            }
    ...
    searchByCriteria: async (criteria: SearchCriteria = {}): Promise<ProduitResponse> => {
        try {
            const query: any = {};
    ...
    getProductsByCategory: async (categoryId: string, limit = 10): Promise<ProduitResponse> => {
        try {
    ...
    getProductsByFournisseur: async (fournisseurId: string): Promise<ProduitResponse> => {
        try {
    ...
    incrementProductViews: async (productId: string): Promise<ProduitResponse> => {
        try {
