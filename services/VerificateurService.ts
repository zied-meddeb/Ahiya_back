...
interface VerificateurResponse {
  ...
  data?:
    | IVerificateur
    | IVerificateur[]
    | { user: IVerificateur; token: string };
}
...
export const verificateurService = {
  ...
  createVerificateur: async (
    verificateurData: Partial<IVerificateur>
  ): Promise<VerificateurResponse> => {
    try {
      const existingVerificateur = await Verificateur.findOne({
        email: verificateurData.email,
      });
      if (existingVerificateur) {
        throw new ServiceError(
          "Verificateur with this email already exists",
          409
        );
      }
