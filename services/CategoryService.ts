...
export const categoryService = {
    ...
    getCategoryByHierarchy: async (hierarchy: string): Promise<CategoryResponse> => {
        try {
            const category = await Category.findOne({ hierarchy });
            if (!category) {
                throw new ServiceError('Category not found by hierarchy path', 404);
            }
            ...
            if (error instanceof ServiceError) throw error;
            throw new ServiceError(`Failed to fetch category by hierarchy: ${error.message}`, 500);
        }
    },
    ...
    createCategory: async (categoryData: Partial<ICategory>): Promise<CategoryResponse> => {
        try {
            ...
            if (error.code === 11000 && error.keyPattern?.hierarchy) {
                throw new ServiceError('A category with the same hierarchy already exists', 409);
            }
            if (error instanceof ServiceError) throw error;
    ...
    updateCategory: async (id: string, categoryData: Partial<ICategory>): Promise<CategoryResponse> => {
        try {
            ...
            if (categoryData.nom) {
                ...
                if (existingCategory) {
                    throw new ServiceError('Another category with this name already exists', 409);
                }
            }
            ...
            if (error.code === 11000 && error.keyPattern?.hierarchy) {
                throw new ServiceError('Another category with the same hierarchy already exists', 409);
            }
            if (error instanceof ServiceError) throw error;
