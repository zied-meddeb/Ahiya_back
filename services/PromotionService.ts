...
export const promotionService = {
    ...
    createPromotion: async (promotionData: Partial<IPromotion>): Promise<PromotionResponse> => {
        try {
            ...
            if (promotion.produits && promotionData.produits && promotionData.produits.length > 0) {
                const produitsData = promotionData.produits;;
                for (const produitData of produitsData) {
                    const createdProduitResponse = await produitService.createProduit(produitData as unknown as IProduit);
                    if (createdProduitResponse.success && createdProduitResponse.data) {
                        ...
                        promotion.produits.push(createdProduit._id as mongoose.Schema.Types.ObjectId)
                    } else {
                ...
                throw new ServiceError('At least one product must be associated with the promotion', 400);
            }
    ...
    getPromotionsByProduit: async (produitId: string): Promise<PromotionResponse> => {
        try {
    ...
    getPromotionsByCategory: async (categoryName: string): Promise<PromotionResponse> => {
        try {
            //find cat by name
            ...
            const products = await Produit.find({ category: category._id }).select('_id');
